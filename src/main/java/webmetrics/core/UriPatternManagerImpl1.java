...
public class UriPatternManagerImpl1 extends AbstractUriPatternManager implements UriPatternManager {

	// proper double wild card at the end (/ctx/my/**)
	private static final String REGEX_DOUBLE_WILD_CARD = "(.*?)\\*\\*(.*)";
	...
	public UriPatternInfo parseUriPattern(String uriPattern) {
		...
		if (!validateWildCard(uriPattern, REGEX_DOUBLE_WILD_CARD)) {
		...
		// check if either double wild card or uri terminator
		result.setDoubleWildCard(uriPattern.endsWith("/**"));
		if (result.isDoubleWildCard())
